# Release signing keys — linux-rhel
...
minisign -G -p platforms/linux-rhel/keys/twdxos-release.pub \
            -s ~/.secure/twdxos-release.key

# per release: sign every fetched file
for f in configs/automatic.conf \
  ...
  minisign -S -s ~/.secure/twdxos-release.key -m "platforms/linux-rhel/$f"
done
git add platforms/linux-rhel/{keys/twdxos-release.pub,**/*.minisig}
```
