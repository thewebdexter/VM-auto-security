...
[[ "$JSON_OUTPUT" == "true" ]] || {
    printf '%b\n' "${CYAN}${BOLD}"
    echo "  ================================================================="
    echo "  TWDxOSOptimisation — Linux (RHEL/Fedora/CentOS) Uninstaller  v${TWDX_VERSION}"
    echo "  ================================================================="
    printf '%b\n' "${NC}"
}
...
run systemctl disable --now dnf-automatic-install.timer 2>/dev/null || true
note_removed "disabled dnf-automatic-install.timer"
...
if [[ -f /etc/ssh/sshd_config.d/99-twdxos-hardening.conf ]]; then
    ...
    run systemctl reload sshd 2>/dev/null || true
...
if grep -q ' # twdxos-hardening$' /etc/fstab 2>/dev/null; then
    ...
    note_removed "fstab mount-hardening lines (reboot to fully revert)"
fi
...
if [[ "$PURGE" == "true" ]] || confirm "Disable fail2ban service?"; then
    run systemctl disable --now fail2ban 2>/dev/null || true
    note_removed "disabled fail2ban"
fi
if command -v firewall-cmd &>/dev/null && systemctl is-active --quiet firewalld; then
    if [[ "$PURGE" == "true" ]] || confirm "Remove twdxos firewalld rules (SSH port, http/https)?"; then
        ...
        note_removed "firewalld http/https services removed (SSH port left in place for safety)"
    fi
fi

# NOTE: chrony/time-sync is an OS baseline and is deliberately left enabled.
...
if [[ "$JSON_OUTPUT" == "true" ]]; then
    ...
    printf '{"tool":"twdxos","platform":"linux-rhel","script":"uninstall","version":"%s","dry_run":%s,"purge":%s,"removed":[%s],"timestamp":"%s"}\n' \
